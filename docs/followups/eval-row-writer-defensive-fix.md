...
**Status (2026-05-08, post-D17b ITEM 4):** All 3 sub-items RESOLVED.
- Sub-item 1 (verdict_reasoning silent truncation): RESOLVED at D17a commit `39c7564`.
- Sub-item 2 (failure_class enum): RESOLVED at D17b ITEM 4.A commit `662311f` — column added via migration `0066_eval_failure_class`; 4-value enum (`none`, `below_threshold`, `critic_flaked`, `agent_raised`); writer + 2 call sites in `evaluate_with_retry` thread the discriminator through.
- Sub-item 3 (writer consolidation): RESOLVED at D17b ITEM 4.B commit `<this commit>` — shared `_try_insert_eval_artifact` helper handles session.add + flush + structlog warning; both writers (`_write_evaluation_row`, `_write_escalation_row`) build their row instance and delegate persistence.
...
- ~~The eval-row schema doesn't distinguish "Critic flaked" from "agent raised before Critic ran" — both land as `total_score=0.0, passed=False`, and the only signal is `critic_reasoning`'s text. A future redesign could add an explicit `failure_class` enum.~~ **RESOLVED at D17b ITEM 4.A (commit `662311f`)**: migration `0066_eval_failure_class` adds a 4-value enum column (`none` / `below_threshold` / `critic_flaked` / `agent_raised`) with a CHECK constraint at the DB layer. `EvalFailureClass` (str-subclass) declared in `evaluation.py`; the two `_write_evaluation_row` call sites in `evaluate_with_retry` supply the discriminator at write time. Pre-D17b rows remain NULL (backfill not load-bearing for forward dashboards). Test `test_eval_failure_class_enum_values_match_db_check_constraint` pins enum/CHECK parity at unit-test time.
- ~~`_write_escalation_row` and `_write_evaluation_row` share enough structure that they could be refactored to a single writer with a discriminator field.~~ **RESOLVED at D17b ITEM 4.B**: extracted `_try_insert_eval_artifact(session, row, *, writer, agent_name, failure_event)` shared helper handling the session.add + flush + structlog warning trail. Both writers now build their row instance (different tables, different columns) and delegate persistence to the helper. Existing log-event contracts (`evaluate.write_evaluation_failed` / `evaluate.write_escalation_failed`) preserved via the `failure_event` parameter so downstream log parsers don't break. Tests at `test_eval_row_writer_none_score.py` cover both the success path and the flush-raises path through the shared helper.

~~None of this blocks D13 ship. Sub-items 2 + 3 deferred — both require schema migration (failure_class enum) or cross-writer refactoring beyond D17a small-fix scope.~~ **All sub-items now RESOLVED (D17a + D17b).**
